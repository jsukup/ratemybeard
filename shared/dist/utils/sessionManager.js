 ...
 */
export async function hasRatedImage(imageId, supabase, sessionId) {
    try {
 ...
 */
export async function getSessionRatings(supabase, sessionId) {
    try {
        const currentSessionId = sessionId || await sessionManager.getSessionId();
        const { data, error } = await supabase
            .from('ratings')
 ...
 */
export async function checkRateLimit(supabase, ipAddress) {
 ...
 */
export async function getSessionStats(supabase, sessionId) {
    try {
        const currentSessionId = sessionId || await sessionManager.getSessionId();
        ...
        const ratingsToday = ratings.filter(r => r.created_at.startsWith(today)).length;
        return {
            totalRatings,
